...
// ================= ระบบค้นหา ตัวกรอง และ การลิงก์จากหน้าแจ้งเตือน =================
$search_text = isset($_GET['q']) ? trim($_GET['q']) : '';
$filter_type = isset($_GET['filter_type']) ? $_GET['filter_type'] : '';
$filter_status = isset($_GET['filter_status']) ? $_GET['filter_status'] : '';
$filter_categories = isset($_GET['category']) ? $_GET['category'] : []; 
$filter_post_id = isset($_GET['post_id']) ? $_GET['post_id'] : ''; // ระบบดึงโพสต์เดียวจากการแจ้งเตือน
...
<div id="commentModal" class="modal">
    <div class="modal-content">
        ...
        <div class="comment-input-area" style="display:flex; gap:10px;">
            ...
            <input type="text" id="commentText" placeholder="Write a comment..." onkeypress="handleEnter(event)" style="flex-grow:1; padding:10px; border-radius:8px; border:1px solid var(--border-color); font-family: inherit; background: rgba(255,255,255,0.05); color:white; outline:none;">
...
// ================= ระบบการแจ้งเตือน (Notifications & Muted) =================
            ...
            list.innerHTML += `
                <div class="notif-item" onclick="window.location.href='index.php?post_id=${n.post_id}'">
                    ...
                    <div id="notif-menu-${n.id}" class="notif-dropdown">
                        <div class="notif-dropdown-item" onclick="event.stopPropagation(); muteUser(${n.actor_id}, '${n.username}')">🔕 Mute notifications from ${n.username}</div>
                        <div class="notif-dropdown-item danger" onclick="event.stopPropagation(); deleteNotification(${n.id})">🗑️ Delete this notification</div>
                    </div>
                </div>
            `;
        });
            ...
            list.innerHTML += `
                <div style="margin-bottom: 5px; display:flex; gap:10px;">
                    ...
                    <div style="flex-grow: 1;">
                        <div style="background:rgba(255,255,255,0.05); border: 1px solid var(--border-color); padding:8px 12px; border-radius:12px; display:inline-block;">
                            <strong style="cursor:pointer; color:var(--primary); font-size:14px;" onclick="openProfileModal(${c.user_id})">${c.username}</strong>
                            <p style="margin:2px 0 0 0; font-size:14px; color:white;">${c.comment_text}</p>
                        </div>
            ...
            if(replyBox) {
                ...
                replyBox.innerHTML += `
                    <div style="margin-bottom: 5px; margin-top: 5px; display:flex; gap:8px;">
                        ...
                        <div>
                            <div style="background:rgba(255,255,255,0.05); border: 1px solid var(--border-color); padding:5px 10px; border-radius:12px; display:inline-block;">
                                <strong style="cursor:pointer; color:var(--primary); font-size:12px;" onclick="openProfileModal(${r.user_id})">${r.username}</strong>
                                <p style="margin:2px 0 0 0; font-size:13px; color:white;">${r.comment_text}</p>
                            </div>
                        </div>
                    </div>
                `;
...
function submitComment() {
    ...
    if(text === '') return;
    let formData = new FormData(); formData.append('post_id', postId); formData.append('comment_text', text);
    ...
    fetch('add_comment.php', { method: 'POST', body: formData }).then(r => r.text()).then(result => {
        if(result === 'success') { cancelReply(); loadComments(postId); } else { alert('Error submitting comment'); }
    });
}
